...
export class TreeView extends EventEmitter {
  ...
  all_expand_close( expand: bool ) {
     function expand_close(node, expand) {
       if (node.className === "toggle") {
          if (node.parentElement.tagName === "LI" && node.parentElement.classList.contains("group")) {
            if (expand) {
                node.parentElement.classList.remove("collapsed");
	    } else {
                node.parentElement.classList.add("collapsed");
	    }
          }
       }
       for (let i = 0; i < node.childNodes.length; i++) {
         expand_close(node.childNodes[i],expand);  
       }
     }
     expand_close( this.treeRoot, expand );
  ...
  private onClick = (event: MouseEvent) => {
    ...
    */
    ...
    if (element.className === "toggle") {
      if (element.parentElement.tagName === "LI" && element.parentElement.classList.contains("group")) {
        if (element.parentElement.classList.contains("collapsed")) {     //expanding
